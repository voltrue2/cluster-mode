...
function startWorker(cb) {
    ...
    process.on('message', function (data) {
        switch (data.command) {
            case CMD.EXIT:
                logger.info('Shutting down worker process for exit:', data.sig);
                shutdown(data.error || null, data.sig);
                return;
            case CMD.RELOAD:
...
function exit(errorExit, sig) {
    ...
    if (numOfWorkers && cluster.isMaster) {
        ...
        async.eachSeries(keys, function (id, next) {
            ...
            msg.send({ command: CMD.EXIT, error: e, sig: sig }, cluster.workers[id]);
        }, function () {
